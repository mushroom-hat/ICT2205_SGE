...
#     # Save the PEM data to a file
#     with open("../\\files_stored_in_candidates3_encrypted_USB\\can3_private_key.txt", "w") as f:
...
def storePrivateKey(private_key):
    ...
    # Save the PEM data to a file
        with open(drive_path + "can3_private_key.txt", "w") as f:
            f.write(str(private_key))
...
def getEllipticParameters(x,y, private_key):
    ...
    with open("can3_ECC_PG.txt", "w") as f:
        f.write(str(x) + " " + str(y))
